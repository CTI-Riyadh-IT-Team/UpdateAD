...
# Excel mapping:
# A = Email
# B = English Name          -> Description
# C = Arabic Name           -> DisplayName
# D = Password
# E = Training Number       -> Name / CN
#                             sAMAccountName
...
function Test-UserRowForReplacement {
    ...
    if (-not [string]::IsNullOrWhiteSpace($User.Email)) {
        $dupEmailRows = @($AllPreparedRows | Where-Object { $_.Email -eq $User.Email })
        if ($dupEmailRows.Count -gt 1) {
            $issues.Add("A: Duplicate email inside users.xlsx")
        }
    }
...
foreach ($user in $users) {
    ...
    try {
        $newUserParams = @{
            # CN / Name uses the training number so it is unique and stable.
            Name              = $user.Username

            # User-facing Arabic name.
            DisplayName       = $user.DisplayName

            # Preserve the English name from column B.
            Description       = $user.EnglishName

            # Login identifiers and student identity used by connected systems.
        ...
        # Read back critical attributes before enabling the account.
        # employeeID is required by SM to recognize/display the student's training number.
        $createdUser = Get-ADUser `
            -Identity $user.Username `
            -Properties employeeID,mail,displayName,description,userPrincipalName `
            -ErrorAction Stop

        if ($createdUser.employeeID -ne $user.Username) {
            throw "Post-create validation failed: employeeID was not saved correctly."
        }

        if ($createdUser.SamAccountName -ne $user.Username) {
            throw "Post-create validation failed: sAMAccountName was not saved correctly."
        }

        if ($createdUser.UserPrincipalName -ne $user.UPN) {
            throw "Post-create validation failed: UserPrincipalName was not saved correctly."
        }

        if ($createdUser.DisplayName -ne $user.DisplayName) {
            throw "Post-create validation failed: DisplayName was not saved correctly."
        }

        if ($createdUser.mail -ne $user.Email) {
            throw "Post-create validation failed: mail was not saved correctly."
        }

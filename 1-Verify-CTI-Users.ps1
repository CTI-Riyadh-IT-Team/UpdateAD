...
# D = Password                 REQUIRED
# E = Training Number/Username REQUIRED -> Name/CN + sAMAccountName + UPN + employeeID
#
# Target:
...
foreach ($user in $users) {
    ...
    # Duplicate email inside the NEW Excel file is treated as a data error.
    if (-not [string]::IsNullOrWhiteSpace($user.Email)) {
        if (@($users | Where-Object { $_.Email -eq $user.Email }).Count -gt 1) {
